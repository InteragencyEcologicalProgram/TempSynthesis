...
heatDays2 <- tempHeat %>%
  ...
  left_join(heatDaysSamp) %>%
  mutate(propDays = round(nDays/totalDays,3)) %>%
  complete(StressAllDay, nesting(Region, Station, fWY), fill = list(propDays=0)) %>%
...
tempHeat2 <- tempDaily %>%
  ...
  mutate(Stress = ifelse(ifelse(Thr21 == lag(Thr21, 1) & Thr21 == lag(Thr21, 2) & Thr21 == 1, "High", "Low"))) %>%
  ungroup() %>%
  group_by(Region, Station, WY, Date) %>%
...
## Temp variables
```{r Models Daily Max}
tempHeat2 <- filter(heat, WY > 2008 & WY < 2020)
...
tempHeat2$fMonth = ordered(tempHeat2$Month)
...
1c. How many days at low stress?
