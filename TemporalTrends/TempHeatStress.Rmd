...
#write.csv(heatDays1, "Data/heatDaysHighMedLow25.csv")
#write.csv(heatDays2, "Data/noRecovery25.csv")
```
...
Temp variables
```{r Models Daily Max}
tempHeat2 <- filter(tempHeat2, WY > 2008 & WY < 2020)
...
tempHeat2M <- left_join(tempHeat2, nMon) %>%
  filter(nMon == 12)
tempHeat2$Station = factor(tempHeat2$Station)
tempHeat2$WYType2_Sac = as.factor(tempHeat2$WYType2_Sac)

#try a sine-transformation on "month" so we can include it in linear models
tempHeat2M$syday = sin(pi*tempHeat2M$yday/365)

# Add season
tempHeat2M <- tempHeat2M %>%
  mutate(Season = ifelse(Month %in% c(12, 1, 2), "Winter", 
                         ifelse(Month %in% c(3,4,5), "Spring", 
                                ifelse(Month %in% c(6,7,8), "Summer",
                                       "Fall"))))
tempHeat2M$Season <- factor(tempHeat2M$Season, levels = c("Spring", "Summer", "Fall", "Winter"))
```
...
```{r}
# Calculate number of samples in a year or water year
heatDaysSamp2 <- tempHeat2M %>%
  group_by(Region, Station, WY, fWY) %>%
  summarize(totalDays = n()) %>%
  filter(totalDays > 250)

# Calculate number of days and proportion of days at each stress level
stressDays1 <- tempHeat2M %>%
  group_by(Region, Station, WY, fWY, Stress) %>%
  summarize(nDays = n()) %>%
    ungroup() %>%
 complete(Stress, nesting(Region, Station, WY, fWY), fill = list(nDays=0)) %>%
...
#write.csv(stressDays1, "Data/heatDaysHighLow21.csv")
#write.csv(stressDays2, "Data/noRecovery21.csv")
```
