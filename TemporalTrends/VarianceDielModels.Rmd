...
ggplot(tempDailyM_s, aes(x = Date, y = rangeTemp)) + geom_jitter(alpha = 0.2) + geom_smooth(se = TRUE)
...
```{r}
ggplot(tempDailyM_3, aes(x = fWY, y = meanRange, group = Region, color = Region)) + geom_point(size = 3) + geom_line() + scale_color_viridis(discrete = TRUE, option = "viridis")  +theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
* Region: High variability in Suisun Marsh, South
...
ggplot(tempHours_sub, aes(Date, Temp, color = TOD)) + geom_point() + facet_wrap(~Region) + scale_color_viridis(discrete=TRUE) + theme_bw()

tempHours_2 <- tempHours %>%
  group_by(fWY, Month, Region,TOD) %>%
  summarize(meanTemp = mean(Temp),
    meanHour = mean(Hour)) %>%
  mutate(Month2 = ifelse(Month<10, paste0("0",Month)) factor(Month))) %>%
  mutate(YrMon = paste0(fWY,Month2))

tempHours_2$Month2

ggplot(tempHours_2, aes(YrMon, meanTemp, color = TOD)) + geom_point() + facet_wrap(~Region) + scale_color_viridis(discrete=TRUE) + theme_bw()
