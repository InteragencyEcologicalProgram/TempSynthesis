...
Questions for Rosie: 
- Can I include region in my model but also have Station as a random variable? 
- When I leave WY as numeric, it comes out significant, but is this accurate? Or is it better to have it as a factor, and say "the last 5 years are higher?"
- We talked about putting year as random variable. Does our question then become is there a difference in winter temperature trends by region, wytype? Or what does the question really become? 
- We don't really care about month specifically, unless there is an interaction of some sort (e.g. we can detect trends for certain months), so should this be a random variable?
- Do we care about season?
- How to incorporate random variables correctly (e.g. nested? crossed? syntax?)
- How to interpret output
- How to compare models (Still AIC?) Look at Zuur
...
Best Model: glm(maxTemp~Region + I(Month^2) + WYType2_Sac +  WYType2_Sac:I(Month^2)
```{r Models Max Monthly Temp}
# WY Type or WY? WYType
summary(M.max1 <- glm(maxAvTemp~Region + I(Month^2) + WYType2_Sac, data = tempMonM))
summary(M.max2 <- glm(maxAvTemp~Region + WY + I(Month^2) , data = tempMonM))
AIC(M.max1, M.max2)
...
# Interactions?
summary(M.max4 <- glm(maxAvTemp~Region + I(Month^2) + WYType2_Sac +  WYType2_Sac:I(Month^2) , data = tempMonM))

summary(M.max5 <- glm(maxAvTemp~Region + I(Month^2) + WYType2_Sac +  WYType2_Sac:I(Month^2) + Region:I(Month^2), data = tempMonM))
...
```{r glmmmax}
summary(M.max1a <- lmer(maxAvTemp~Region + I(Month^2) + WYType2_Sac + (1|Station), data = tempMonM, na.action = na.omit))

summary(M.max1b <- lmer(maxAvTemp~Region + I(Month^2) + WYType2_Sac + (1|Year) + (1|Station) , data = tempMonM, na.action = na.omit))
...
summary(M.max5a <- lmer(maxAvTemp~Region + I(Month^2) + WYType2_Sac + WYType2_Sac:I(Month^2) + (1|Station), data = tempMonM, na.action = na.omit))

summary(M.max5b <- lmer(maxAvTemp~Region + I(Month^2) + WYType2_Sac + WYType2_Sac:I(Month^2) + (1|Year) + (1|Station) , data = tempMonM, na.action = na.omit))
...
Scientific Questions: 
1. Do we see an annual trend in minimum temperature by month? 
...
### Normal GLM
    Best Model: glm(minTemp~Region + Season + WYType2_Sac + Year + WYType2_Sac:Season)
```{r glmMin}
...
### Mixed Model - Year as random factor
Choice: M.min2b (Season:WYType) even though AIC very slightly higher
```{r glmmMin}
...
summary(M.meanW1 <- glm(meanTemp~Region + I(Month^2) + WYType2_Sac, data = Winter))
summary(M.meanW2 <- glm(meanTemp~Region + I(Month^2) + WY , data = Winter))

# Year as factor
summary(M.meanW3 <- glm(meanTemp~Region + I(Month^2) + fWY, data = Winter))
...
```{r}
summary(M.meanW4a <- lmer(meanTemp~Region + I(Month^2) + (1|fWY), data = Winter, na.action = na.omit))

summary(M.meanW4b <- lmer(meanTemp~Region +  I(Month^2) + (1|fWY) + (1|Station), data = Winter, na.action = na.omit))
...
summary(M.minW1 <- glm(minAvTemp~Region + I(Month^2) + WYType2_Sac, data = Winter))
summary(M.minW2 <- glm(minAvTemp~Region + I(Month^2) + WY , data = Winter))

# Year as factor
summary(M.minW3 <- glm(minAvTemp~Region + I(Month^2) + fWY, data = Winter))

# Month as factor
summary(M.minW4 <- glm(minAvTemp~Region + fMonth + fWY, data = Winter))
...
### Conclusions
* Region: Average min temps lower in North Delta, South, Suisun Marsh
* Month: Nov>Feb>Dec>Jan
* WY 2015 very warm, last 5 years warmer average min temps.
...
```{r}
summary(M.minW4a <- lmer(minAvTemp~Region + I(Month^2) + (1|fWY), data = Winter, na.action = na.omit))

summary(M.minW4b <- lmer(minAvTemp~Region +  I(Month^2) + (1|fWY) + (1|Station), data = Winter, na.action = na.omit))
...
ggpredict(M.minW5f, terms = c("fWY", "fMonth"), type = "re") %>% 
   plot() +
   labs(x = "Water Year", y = "Month", title = "Avg Min Temp") + 
   theme_minimal()
