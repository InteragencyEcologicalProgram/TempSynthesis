...
tempMon <- readRDS(here("Data", "tempMonthly.rds"))
...
# Month and Region
max1a <- ggplot(tempMonM) + 
  geom_jitter(aes(x = fWY, y = maxAvTemp, color = fMonth), size = .9) +
  facet_wrap(~Region) + 
  scale_color_viridis(option = "viridis", discrete = TRUE) + 
...
tempMonTile <- tempMonM %>%
  group_by(fWY, Region) %>%
  summarize(meanMaxTemp = mean(maxAvTemp))

ggplot(tempMonTile) + 
  geom_tile(aes(x = fWY, y = Region, fill = meanMaxTemp)) +
...
# WYType:Month
ggplot(Summer) + 
  geom_boxplot(aes(x = Month, y = maxAvTemp, fill = WYType2_Sac)) + 
    labs(title = "WYType:Month") +
  scale_fill_viridis(option = "viridis", discrete = TRUE) + theme_bw()
