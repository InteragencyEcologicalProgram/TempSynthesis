...
## Read in data
```{r}
tempMonthly <- readRDS(here::here("Data", "tempMonthly_20220801.rds"))
tempDaily <- readRDS(here::here("Data", "tempDaily_20210811.rds"))
# tempMonthly <- readRDS(here("Data", "tempMonthly_20230412.rds"))
...
tempTable <- tempDaily %>% left_join(LatLon) %>%
  ...
  mutate(Region = case_when(Region == "Sac River" ~ "Confluence",
                            ...
                            TRUE ~ Region))
# write_csv(tempTable, "Tables/Stations_included_10year.csv")
```
...
VER <- temp_qr %>% filter(Station == "VER")
...
staOfInterest <- temp_qr %>%
  ...
  filter(Station%in% c("VER", "SJR", "TEA", "BLL")) %>%
...
ggplot(VER) + geom_point(aes(Date, maxDaily)) + theme_bw() +
...
(range_plot <- ggplot(staOfInterest) + 
  ...
  labs(y = "Daily Temperature Range (°C)") +
  theme(strip.text = element_text(size = 11),
        axis.title.x = element_blank()) )

png(filename = "Figures/Range_plot.png", height = 5, width = 7,units = "in", 
...
```{r}
temp <- tempMonthly  %>%
  filter(!Station %in% c("VER", "RPN", "RIP")) %>%
...
### model validation
```{r}
png(filename = "Figures/plots_residuals_maxmodel.png", width = 9, height = 7, units = "in", res = 300)
par(mfrow = c(2,4))
...
resid_df <- data.frame(resid.maxs9)
res_max <- cbind(temp, resid_df)
dat <- ggeffects::ggpredict(m.maxs9)
fitted(m.maxs9, level = 0:1)
...
(d_p1 <- ggplot(res_max) + geom_point(aes(fitted, resid.maxs9)) +
...
#(d_p3 <- acf(resid))
(d_p3 <- acf_d)
(d_p4 <- ggplot(res_max) +  geom_point(aes(flow, resid.maxs9)) +
    labs(x = "Log Flow (cfs)", y = "Residuals")+
    theme_vis)
(d_p5 <- ggplot(res_max) +  geom_point(aes(WTmwk, resid.maxs9)) +
    labs(x = "Mean Weekly Water Temperature (°C)", y = "Residuals")+
    theme_vis)
(d_p6 <- ggplot(res_max) +  geom_boxplot(aes(inund_fac, resid.maxs9)) +
...
plotmax9 = plot(max9.emm, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.001)
results_max92 = cld(max9.emm, Letters = LETTERS, alpha = 0.001, p.adj.methods = "bonferroni") %>%
  left_join(plotmax9) %>%
  mutate(.group = gsub(" ", "", .group))

results_max9.emm = as.data.frame(pairs(max9.emm, adjust = "bonferroni", alpha = 0.001)) %>%
  mutate(sig = ifelse(p.value<0.01, "yes",  "no")) %>%
  arrange(p.value)

# Region/Season ------------------------------
max9.emm2 <- emmeans(m.maxs9, specs = pairwise ~ Region2:Season)
# max9.emm2 <- emmeans(m.maxs9, Region2:Season)
# plotmax9.rs = plot(max9.emm2, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.001)

# add letters
letters.rs <- cld(max9.emm2, Letters = LETTERS, alpha = 0.001, p.adj.methods = "bonferroni")%>%
  mutate(.group = gsub(" ", "", .group))
ci_max9_rs <- as.data.frame(max9.emm2$emmeans) %>%
...
plotmax9.rs <- emmeans(m.maxs9, ~ Region2*Season, adjust = "bonferroni", alpha = 0.001)
...
### Year plot
```{r}
(plot_ci_max9_wy <- ggplot(data = results_max92,) + 
...
(plot_ci_max9 <- ggplot() + 
   geom_errorbar(data = ci_max9_rs, aes(x = Season, ymin = asymp.LCL, ymax = asymp.UCL, color = Region2), size = 0.5,  width = 0.75, position = position_dodge2()) +
  geom_text(data = ci_max9_rs, aes(label = .group, x = Season, y = asymp.UCL + 0.5), position = position_dodge2(width = 0.75)) + 
...
```{r}
library(patchwork)
p4 <- ggplot(data.frame(l = "Maximum Water Temperature (°C)\nwith 95% Confidence Intervals", x = 1, y = 1)) +
...
png(filename = "Figures/Watertemp_trends_max3.png", height = 8.5, width = 7,units = "in", 
    res = 300, pointsize = 12, family = "sans")
...
#save a dataframe of all the things the plotting function uses so we can plot with ggplot later
plotmin9 = plot(min9.emm, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.001)
results_min92 = cld(min9.emm, Letters = LETTERS, alpha = 0.001, p.adj.methods = "bonferroni") %>%
  left_join(plotmin9) %>%
  mutate(.group = gsub(" ", "", .group))%>%
   mutate(across(emmean:asymp.UCL,~round(.x, 3))) 

results_min9.emm = as.data.frame(pairs(min9.emm, adjust = "bonferroni", alpha = 0.001)) %>%
  mutate(sig = ifelse(p.value<0.01, "yes",  "no")) %>%
  arrange(p.value)
...
# plotmin9.rs = plot(min9.emm2, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.001)

letters.rs.min <- cld(min9.emm2, Letters = LETTERS, alpha = 0.05, p.adj.methods = "bonferroni")%>%
  mutate(.group = gsub(" ", "", .group))
ci_min9_rs <- as.data.frame(min9.emm2$emmeans) %>%
...
```{r}
library(patchwork)
p4 <- ggplot(data.frame(l = "Minimum Water Temperature (°C) with 95% Confidence Intervals", x = 1, y = 1)) +
      geom_text(aes(x, y, label = l), angle = 90) + 
      theme_void() +
      coord_cartesian(clip = "off")
...
png(filename = "Figures/Watertemp_trends_min.png", height = 7.5, width = 7,units = "in", 
    res = 300, pointsize = 12, family = "sans")
...
#save a dataframe of all the things the plotting function uses so we can plot with ggplot later
plotmean9 = plot(mean9.emm, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.001)
results_mean92 = cld(mean9.emm, Letters = LETTERS, alpha = 0.001, p.adj.methods = "bonferroni") %>%
  left_join(plotmean9) %>%
  mutate(.group = gsub(" ", "", .group))%>%
   mutate(across(emmean:asymp.UCL,~round(.x, 3))) 

results_mean9.emm = as.data.frame(pairs(mean9.emm, adjust = "bonferroni", alpha = 0.001)) %>%
  mutate(sig = ifelse(p.value<0.01, "yes",  "no")) %>%
  arrange(p.value)
...
plotmean9.rs = plot(mean9.emm2, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.001)

# add letters
letters.rs <- cld(mean9.emm2, Letters = LETTERS, alpha = 0.05, p.adj.methods = "bonferroni")%>%
  mutate(.group = gsub(" ", "", .group))
...
```{r}
mean9.emm <- emmeans(m.means9, "fWY")
pairs(mean9.emm, adjust = "tukey")
plot(mean9.emm, comparisons = TRUE) + theme_bw()
ci_mean9_wy <- confint(mean9.emm)
results_mean9.emm = as.data.frame(pairs(mean9.emm)) %>%
  mutate(sig = ifelse(p.value<0.01, "yes",  "no")) %>%
  arrange(p.value)
# 
# ci_mean9_wy <- as.data.frame(confint(mean9.emm))
# ci_mean9_wy <- ci_mean9_wy %>%
#   mutate(Label = c("a", "b", "ac", "d", "e", "g", "e", "f", "h", "cf"))


mean9.emm2 <- emmeans(m.means9, specs = pairwise ~ Region2:Season)
pairs(mean9.emm2, adjust = "bonferroni")
results_mean9.emm2 = as.data.frame(pairs(mean9.emm2)) %>%
  mutate(sig = ifelse(p.value<0.01, "yes",  "no")) %>%
  arrange(p.value)
#multcomp::cld(pairs(mean9.emm2, adjust = "bonferroni"))

ci_mean9_rs0 <- as.data.frame(mean9.emm2$emmeans)
ci_mean9_rs <- ci_mean9_rs0 %>%
  mutate(Label = c("ab", "a", "bc", "abc", "abc", "c", "de", "d", "f", "ef", "g", "d")) %>%
  mutate(Season = ifelse(Season == "Wet", "Wet Season", ifelse(Season == "Dry", "Dry Season", NA))) %>%
  mutate(Season = factor(Season, levels = c("Wet Season", "Dry Season")))
...
```{r}
library(patchwork)
p4 <- ggplot(data.frame(l = "Mean Water Temperature (°C) with 95% Confidence Intervals", x = 1, y = 1)) +
      geom_text(aes(x, y, label = l), angle = 90) + 
      theme_void() +
      coord_cartesian(clip = "off")
...
png(filename = "Figures/Watertemp_trends_mean.png", height = 7.5, width = 7,units = "in", 
    res = 300, pointsize = 12, family = "sans")
...
write_csv(ci_max9_rs, "TemporalTrends/emmeans_results/ci_rs.csv")
write_csv(results_max92, "TemporalTrends/emmeans_results/ci_wy.csv")

write_csv(ci_min9_rs, "TemporalTrends/emmeans_results/ci_rs_min.csv")
write_csv(results_min92, "TemporalTrends/emmeans_results/ci_wy_min.csv")

write_csv(ci_mean9_rs, "TemporalTrends/emmeans_results/ci_rs_mean.csv")
write_csv(results_mean92, "TemporalTrends/emmeans_results/ci_wy_mean.csv")
