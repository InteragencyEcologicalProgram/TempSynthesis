...
plotmax9 = plot(max9.emm, comparisons = TRUE, plotit = F)


#the cld function from the multcomp package gives you the letters
library(multcomp)

#this organzies all the results
results_max92 = cld(max9.emm, Letters = LETTERS) %>%
  left_join(plotmax9)
...
(plot_ci_max9_wy <- ggplot(data = results_max92,) + 
   ...
   geom_text( aes(label = .group, x = fWY, y = asymp.UCL + 0.5), position = position_dodge2(width = 0.75)) + 
  geom_point( aes(x=fWY, y=emmean), size = 2.5, shape = 5) + 
     geom_segment(aes(x=fWY, y=emmean,xend =fWY, yend = rcmpl), color = "red",  linewidth = 2, 
                  arrow = arrow(length = unit(.1,"inches"), ends = "last"))+
  geom_segment(aes(x=fWY, y=emmean,xend = fWY, yend = lcmpl), color = "red",   linewidth = 2,
               arrow = arrow(unit(.1,"inches"), ends = "last"))+
