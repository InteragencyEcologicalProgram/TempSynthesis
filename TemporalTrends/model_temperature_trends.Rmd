...
tempMonthly <- readRDS(here("Data", "tempMonthly.rds"))
...
plotmax9.rs = plot(max9.emm2, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.01)

# add letters
letters.rs <- cld(max9.emm2, Letters = LETTERS, alpha = 0.05, p.adj.methods = "bonferroni")%>%
  mutate(.group = gsub(" ", "", .group))
...
```{r}
library(patchwork)
p4 <- ggplot(data.frame(l = "Max Water Temperature (°C) with 95% Confidence Intervals", x = 1, y = 1)) +
...
plotmin9.rs = plot(min9.emm2, comparisons = TRUE, plotit = F, adjust = "bonferroni", alpha = 0.01)

# add letters
letters.rs <- cld(min9.emm2, Letters = LETTERS, alpha = 0.05, p.adj.methods = "bonferroni")%>%
  mutate(.group = gsub(" ", "", .group))
...
(plot_ci_min9_wy <- ggplot(results_min92) + 
   geom_errorbar(aes(x = fWY, ymin = asymp.LCL, ymax = asymp.UCL), size = 1,  width = 0.3) +
      geom_text( aes(label = .group, x = fWY, y = asymp.UCL + 0.3), position = position_dodge2(width = 0.75)) + 
  # geom_segment(aes(x=fWY, y=emmean, xend =fWY, yend = rcmpl), color = "orange",
  #              linewidth = 3, arrow = arrow(length = unit(.1,"inches"),
  #                                           ends = "last"))+
...
png(filename = "Figures/Watertemp_trends_min.png", height = 7, width = 7,units = "in", 
    res = 300, pointsize = 12, family = "sans")
