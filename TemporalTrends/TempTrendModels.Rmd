...
Edit dataset for Peggy
```{r}
StationYearCombo2 <- tempFilt %>%
  group_by(Region, Station, Year,  Date) %>%
  summarize(perDay = n()) %>%
  ungroup() %>%
...
StationNyears2 <- StationYearCombo2 %>%
  filter(totalDays>250) %>%
  group_by(Region, Station) %>%
...
DataYrSubset2 <- filter(StationNyears2, nYear > 9 & lastYear == 2019) %>%
  left_join(StationYearCombo2) 

tempToUseCalYr <- inner_join(tempFilt, DataYrSubset2, by = c("Region","Station", "Year")) %>%
  filter(Region != "Far North") %>%
  filter(totalDays > 250)


today = format(today(), "%Y%m%d")
#saveRDS(tempToUseCalYr, paste0("Data/temp11yearsCalYr_", today, ".rds"))
