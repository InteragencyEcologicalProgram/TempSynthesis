...
tempDaily8Summary <- tempDaily8b %>%
  group_by(fWY, Region, Region2, Station) %>%
  summarize(meanMaxTemp = mean(maxDaily)) %>%
  ungroup()

tempSeason8SummaryStation <- tempDaily8b %>%
  group_by(Season, fWY, WY, Region, Region2,Station) %>%
  summarize(meanMaxTemp = mean(maxDaily),
            sd = sd(maxDaily),
            n = n(),
...
tempSeason8Summary <- tempDaily8b %>%
  group_by(Season, fWY, WY, Region, Region2) %>%
  summarize(meanMaxTemp = mean(maxDaily),
            sd = sd(maxDaily),
            n = n(),
...
## Lines
```{r}
(line8yr <- ggplot(tempSeason8Summary) + 
...
```{r}
tiff(filename=file.path("Figures/Trends8Years_line.tiff"), units="in",type="cairo", bg="white", height=5.5, 
     width=7, res=300, pointsize=12,compression="lzw")
line8yr
dev.off()
```
