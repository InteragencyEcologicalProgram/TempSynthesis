...
```{r}
tempSum <- tempMonM %>% filter(Month %in% c(5,6,7,8,9,10)) %>%
  ...
  ungroup()
tempWet <- tempMonM %>% filter(Month %in% c(11,12,1,2,3,4)) %>%
    group_by(Region, Station, WY, WYType2_Sac) %>%
  summarize(maxT = mean(maxTemp),
            meanT = mean(meanTemp),
            minT = mean(minTemp),
...
```{r, fig.dim = c(7,8)}
pheatmap(tempSum_yearorder, scale = "column", annotation_row = row_annotSum, cluster_cols = F, main = "Summer Means scaled by column")
```


```{r fig.dim = c(7,8)}
pheatmap(tempSum_yearorder, scale = "row", annotation_row = row_annotSum,  annotation_col = col_annotSum,
         cluster_cols = F, main = "Summer Means scaled by row")
```
...
pheatmap(tempWet_yearorder, scale = "column", annotation_row = row_annotWet,
         ...
         cluster_cols = F, main = "Wet Season Means scaled by year")
```


```{r fig.dim = c(7,8)}
pheatmap(tempWet_yearorder, scale = "row", annotation_row = row_annotWet,  annotation_col = col_annotWet,
         cluster_cols = F, main = "Wet Season Means scaled by station")
...
pheatmap(tempWetWide1, scale = "row", annotation_col = col_annotWet, annotation_row = row_annotWet,
         main = "Scaled by Station - Look at year differences")
```


```{r fig.dim = c(7,8)}
pheatmap(tempWetWide1, scale = "column", annotation_col = col_annotWet, annotation_row = row_annotWet,
         main = "Scaled by Year - Look at station differences")
```
