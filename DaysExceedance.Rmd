...
tempRegions <- tempDaily %>%
  mutate(Region = replace(Region, Region == "Far North", "North Delta"),
         Region = replace(Region, Region == "San Joaquin", "Central Delta"),
...
Bind data together
```{r}
DaysExceedanceData0 <- bind_rows(DaysExceedanceBox) %>% distinct() # Not sure why it's creating so many extra rows. For now just apply distinct. 

level_order = c("Green Sturgeon", "White Sturgeon","Delta Smelt", "Longfin Smelt", "Wakasagi","Chinook Salmon", "Steelhead/RB Trout","Splittail", "Silverside", "Striped Bass", "Largemouth Bass", "Arundo", "Brazilian Waterweed", "Water Hyacinth", "Microcystis", "Potamocorbula")
...
DaysExceedanceData <- DaysExceedanceData0 %>%
  dplyr::mutate(daysExceedanceOpt = replace(daysExceedanceOpt, is.na(SubOptimum), NA),
         ...
         Species = case_when(Species == "STB" ~ "Striped Bass",
                             ...
                             Species == "SteelheadRBT" ~ "Steelhead/RB Trout",
...
(FishTol1 <- 
   ...
   geom_vline(aes(xintercept = stage(Species, after_scale = 11.5)), linetype = "dashed", size = 1, color = "palegreen4") +
    geom_boxplot(aes(x = Species,  y = daysExceedanceTol, fill = LifeStage), position = position_dodge(preserve = "single")) +
   annotate(geom = "text", x = 6, y = 300, label = "Fish", vjust = 1, size = 5) +
   annotate(geom = "text", x = 14, y = 300, label = "Vegetation & Clams", vjust = 1, size = 5) +
...
(FishOpt1 <- 
   ...
   geom_rect(xmin = 14.5, xmax = 15.5, ymin = -Inf, ymax = Inf, fill = "gray85") +
      geom_vline(aes(xintercept = stage(Species, after_scale = 11.5)), linetype = "dashed", size = 1, color = "palegreen4") +
      geom_boxplot(aes(x = Species, y = daysExceedanceOpt, fill = LifeStage), position = position_dodge(preserve = "single")) +
    annotate(geom = "text", x = 6, y = 300, label = "Fish", vjust = 1, size = 5) +
   annotate(geom = "text", x = 14, y = 300, label = "Vegetation & Clams", vjust = 1, size = 5) +
   viridis::scale_fill_viridis(discrete = TRUE, option = "turbo") + 
   scale_y_continuous(limits = c(0,300), breaks = seq(from = 0, to = 300, by = 25)) +
