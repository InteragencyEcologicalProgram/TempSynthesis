...
thresholds <- readxl::read_excel(here::here("SpeciesTables", "SpeciesThresholds.xlsx")) %>%
  na_if(.,"NA") 
species <- readr::read_csv(here::here("SpeciesTables", "absence_presence_temp.csv")) 
```
...
daysExceedanceSummary <- DaysExceedanceDataUpper %>%
  ...
  summarize(meanTolerance = round(mean(daysExceedanceTol, na.rm = TRUE),0),
            minTolerance= min(daysExceedanceTol, na.rm = TRUE),
            maxTolerance =  max(daysExceedanceTol, na.rm = TRUE),
            meanSuboptimum = round(mean(daysExceedanceOpt, na.rm = TRUE),0),
            minSuboptimum= min(daysExceedanceOpt, na.rm = TRUE),
            maxSuboptimum =  max(daysExceedanceOpt, na.rm = TRUE)) %>%
  mutate(outputTolerance = paste0(meanTolerance, " (", minTolerance, "-", maxTolerance, ")"),
         outputSuboptimum = paste0(meanSuboptimum, " (", minSuboptimum, "-", maxSuboptimum, ")"))

write_csv(daysExceedanceSummary, here::here("SpeciesTables","Table_numDaysExceedance.csv"))            
```
