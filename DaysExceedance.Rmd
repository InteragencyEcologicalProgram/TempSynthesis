...
species0b <- readr::read_csv(here::here("SpeciesTables", "exportfishabsence_presence.csv")) 
species0 <- readr::read_csv(here::here("SpeciesTables", "absence_presence_temp.csv")) %>%
...
f_threshold_plot <- function(df, var) {
  ggplot(df) + 
   ...
   annotate(geom = "text", x = 10, y = 300, label = "Fish", vjust = 1, hjust = 0.5, size = 4) +
...
```{r}
# write_csv(DaysExceedanceDataUpper, "SpeciesTables/daysExceedanceUpperDataForPlotting.csv")
DaysExceedanceDataUpper <- read_csv("SpeciesTables/daysExceedanceUpperDataForPlotting.csv")
fieldtemps <- read_csv("SpeciesTables/surveymaxtemps_v3.csv") %>%
  select(Taxon = Taxa,
         LifeStage,
         q3, maxTemp)
lengths <- read_csv("publish_thresholds/thresholds_edi/data_objects/Lifestage_length_designations.csv") %>%
  ...
  select(Species, LifeStage, Length_mm, Length_type)
```
...
daysExceedanceSummary <- DaysExceedanceDataUpper %>%
  ...
  distinct() %>%
  left_join(thresholds %>% select(Species, LifeStage, Taxon, Suboptimum, Tolerance))
...
summary_all <- left_join(daysExceedanceSummary, fieldtemps, by = c("Taxon", "LifeStage")) %>%
 ...
 dplyr::select(Species, Taxon, LifeStage, Length_mm, Length_type,
               ...
               `Suboptimum Temperature Threshold (°C)` = Suboptimum, 
               `Tolerance Temperature Threshold (°C)` = Tolerance, 
               outputSuboptimum, outputTolerance) %>%
  arrange(Species, LifeStage)

write_csv(summary_all, here::here("SpeciesTables","Table_AppendixB.csv"))
write_csv(daysExceedanceSummary, here::here("SpeciesTables","Table_numDaysExceedance_v3.csv"))    
