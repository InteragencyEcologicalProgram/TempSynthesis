...
#### Save.load files
```{r}
save(delta2, stas, newdatag21abin, newdatag25abin, newdatag21NRabin,  newdatag25NRabin, file = "newdataPreds_annualanomaly_20211202.RData")

load("newdataPreds_annualanomaly_20211202.RData")
```
...
#### Save raster files
```{r}
save(rastered_preds21abin, rastered_preds25abin, rastered_preds21NRabin, rastered_preds25NRabin, file = "RasteredPreds_20211202.RData")
```

#### Start here if just want to plot
```{r}
load("deltabuffThick.RData")
load("RasteredPreds_20211202.RData")
```
...
Save crops
```{r}
save(raster25Diff_crop, raster21Diff_crop, raster21NRDiff_crop, raster25NRDiff_crop, file = "RasterDiff_20211202.RData")

load("RasterDiff_20211202.RData")
```
...
#### Save and load here
```{r}
save(plot21_anom, plot21_avg, plot25_anom, plot25_avg, plot21NR_anom, plot21NR_avg, plot25NR_anom, plot25NR_avg, raster21DiffPlot, raster25DiffPlot, raster21NRDiffPlot, raster25NRDiffPlot, file= "finalplots_20211202.RData")

load("finalplots_20211202.RData")
```
...
```{r}
tiff(filename=file.path("Figures/StressNRPlot.tiff"), units="in",type="cairo", bg="white", height=3.5, 
     width=8, res=300, pointsize=10,compression="lzw")
...
Info25 <- noRecov25 %>%
  ...
  mutate(Region = factor(Region, levels = c("Far North", "North Delta", "Confluence", "Central", "South", "Suisun Bay", "Suisun Marsh")))
...
```{r}
tiff(filename=file.path("Figures/Anomaly25NRPlot.tiff"), units="in",type="cairo", bg="white", height=4, 
