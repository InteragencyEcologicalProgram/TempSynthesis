...
```{r}
load("deltabuffThick.RData")
load("RasteredPreds_20210714.RData")
```
...
raster21Diff$Prediction2 = raster21Diff$Prediction *365

raster25Anom0 <- rastered_preds25abin[[1]]
raster25Anom1 <- rastered_preds25abin[[2]]
raster25Diff <- raster25Anom1-raster25Anom0 
raster25Diff$Prediction2 = raster25Diff$Prediction *365

raster_plotDayDiff(raster25Diff)

raster25Diff_crop <- st_crop(raster25Diff, deltabuff)
raster25DiffDays <- raster25Diff_crop[2]
ggplot()+
    geom_stars(data=raster25DiffDays)+
   scale_x_continuous(breaks = seq(from = 550000, to = 670000, by = 40000))+
  scale_fill_viridis_c(option = "plasma", breaks = seq(from = 0, to = 30, by = 5), na.value="white",limits = c(0, 30))+
   # facet_wrap(~Date)+
    coord_sf()+
    ylab("Latitude")+
    xlab("Longitude")+
    ...
    #guides(fill = guide_colorbar(barwidth = 2))
        guides(fill=guide_colorbar(title="Difference in Days"))
...
raster21DiffDays <- raster21Diff_crop[2]
ggplot()+
    geom_stars(data=rasterDiffDays)+
   scale_x_continuous(breaks = seq(from = 550000, to = 670000, by = 40000))+
  scale_fill_viridis_c(option = "plasma", breaks = seq(from = 0, to = 30, by = 5), na.value="white",limits = c(0, 30))+
   # facet_wrap(~Date)+
    coord_sf()+
...
plot21NR_avg <- raster_plot(rastered_preds21NRabin, 1) + ggtitle("Average year")
plot21NR_anom <- raster_plot(rastered_preds21NRabin, 2) + ggtitle("Warm year (+1C)")
...
```{r}
save(plot21_anom, plot21_avg, plot25_anom, plot25_avg, plot21NR_anom, plot21NR_avg, plot25NR_anom, plot25NR_avg, file = "finalplots_20210809.RData")

save(plot21_anom, plot21_avg, plot25_anom, plot25_avg, file = "finalplotsPart1_20210809.RData")
```
