...
noRecov210 <- read_csv("Data/noRecovery21_20210904.csv")
...
#### Save.load files
```{r}
save(delta2, newdatag21abin, newdatag25abin, newdatag21NRabin,  newdatag25NRabin, file = "newdataPreds_annualanomaly_20210929.RData")

load("newdataPreds_annualanomaly_20210929.RData")
```
...
raster_plotDayDiff<-function(data, labels="All"){
 ...
 ggplot()+
  geom_stars(data=raster21Diff_crop)+
...
(plot21_avg3 <- annotate_figure(plot21_avg2, left = text_grob("21C", color = "black", rot = 90)))
plot25_avg3 <- annotate_figure(plot25_avg2,
                left = text_grob("25C", color = "black", rot = 90))
...
(raster21DiffPlot2 <- raster21DiffPlot + theme(legend.position = "none") +
    labs(title = "Difference"))
(raster25DiffPlot2 <- raster25DiffPlot + theme(legend.position = "none"))


(stressPlotC <- grid.arrange(legend1, legend2, plot21_avg3, plot21_anom2, raster21DiffPlot2,
                            plot25_avg3, plot25_anom2, raster25DiffPlot2,
                            nrow = 3,
...
(plot21NR_avg3 <- annotate_figure(plot21NR_avg2, left = text_grob("21C", color = "black", rot = 90)))
