...
range(newdatag25abin$Prediction*365) #0.218-319.6
...
Range <- Info25 %>%
  ...
  summarize(min = min(nDaysAdj),
            max = max(nDaysAdj))
Anomaly25NRPlot <- ggplot(Info25) + geom_boxplot(aes(x = Region, y = nDays, fill = AnomalyGrp)) + labs(y = "Number of Days", fill = "Anomaly") + scale_fill_viridis(discrete = TRUE, option = "cividis") + theme_bw() + theme(axis.text.x = element_text(angle = 90)) 
```
