...
map_final <- 
  ...
  geom_sf(data = WW_Delta_crop, fill = "steelblue", colour = "steelblue", alpha = 0.6, inherit.aes = FALSE) + 

  geom_sf(data = filter(WW_Delta,HNAME == "SACRAMENTO RIVER"), fill = "navyblue", colour = "navyblue", alpha = 0.6, inherit.aes = FALSE) + 
  geom_sf(data = filter(WW_Delta,HNAME == "SAN JOAQUIN RIVER"), fill = "cadetblue1", colour = "cadetblue1", alpha = 0.8, inherit.aes = FALSE) +  
  ...
  geom_label(data = centroid_coords, aes(label = Region, x = X, y = Y), inherit.aes = FALSE, 
             nudge_y = ifelse(centroid_coords$Region %in% c("Suisun Marsh"), 0.15, ifelse(centroid_coords$Region == "Suisun Bay", -0.1, ifelse(centroid_coords$Region == "South", 0.05, ifelse(centroid_coords$Region == "Central", 0.08, 0)))), 
             nudge_x = ifelse(centroid_coords$Region %in% c("North Delta", "Central"), 0.3,ifelse(centroid_coords$Region == "Far North", 0.25, ifelse(centroid_coords$Region == "South", 0.2, ifelse(centroid_coords$Region == "Confluence", 0.35, 0)))))+
  #geom_label_repel(data = rivers_4269, aes(x = Longitude, y = Latitude, label = River), arrow = arrow(length = unit(0.03, "npc"), type = "closed", ends = "last"), nudge_x = 0.15,  inherit.aes = FALSE, color = c("navyblue", "darkslategray4") ) +
...
gg_inset_map1 = ggdraw() +
  draw_plot(map_final) +
  draw_plot(inset, x = 0.23, y = 0.68, width = 0.3, height = 0.28)
...
```{r}
gg_inset_map1
ggsave("Figures/StationMapInset4.tiff", width = 8, height = 8, device = 'tiff', dpi = 300, bg = "transparent")
