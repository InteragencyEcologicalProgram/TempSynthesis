...
inset <- ggplot() + 
  ...
  geom_sf(data = insetbbox, fill = NA, color = "red", size = 1.1) +
  theme_void()
...
map_final <- ggmap(map_obj) + 
  ...
  geom_sf(data = stations_sf_4269, size = 4, shape = 21, 
          aes(fill = Region), inherit.aes = FALSE)+
  geom_sf(data = regions_4269, aes(fill = Region), inherit.aes = FALSE, alpha = 0.2)+
...
gg_inset_map1 = ggdraw() +
  draw_plot(map_final) +
  draw_plot(inset, x = 0.14, y = 0.68, width = 0.3, height = 0.3)
...
```{r}
gg_inset_map1
ggsave("Figures/StationMapInset.jpeg", width = 8, height = 8, device = 'png', dpi = 300, bg = "transparent")
