...
tempmean = temps %>%
  ...
  summarize(Tempave = mean(Temp, na.rm = T), TempMax = max(Temp, na.rm = T), TempMin = min(Temp, na.rm = T))
...
g3 = bam(Temp ~ s(julian, bs="cc" ) + s(Year) + 
...
g4 = bam(Temp ~ s(julian, bs = "cc") + s(Year) + 
           ti(Latitude, Longitude, Year)+ 
           ti(Latitude, Longitude, julian), 
