...
## plot categories
```{r}
ggplot(tol, aes(x = LS, y = Tol_upper)) + geom_boxplot()
ggplot(tol, aes(x = LS, y = Tol_lower)) + geom_boxplot()
...
## plot categories
```{r}
ggplot(tol_days, aes(x = LS, y = days_upper)) + geom_boxplot()
ggplot(tol_days, aes(x = LS, y = days_lower)) + geom_boxplot()
...
## t-test: significant
```{r}
t.test(tol_days$LS2, tol_days$days_upper)
t.test(tol_days$LS2, tol_days$days_lower)
```
...
## lm
```{r}
mLSd1 <- lm(days_upper ~ LS, data = tol_days)
summary(mLSd1)
anova(mLSd1)
par(mfrow = c(2,2))
plot(mLSd1)
...
mLSd2 <- lm(days_lower ~ LS, data = tol_days)
summary(mLSd2)
anova(mLSd2)
par(mfrow = c(2,2))
plot(mLSd2)
...
## lmer by species
```{r}
mLSd3 <- lmer(days_upper ~ LS + (1|Species), data = tol_days)
summary(mLSd3)

mLSd4 <- lmer(days_lower ~ LS + (1|Species), data = tol_days)
summary(mLSd4)
```
