...
This is the most recent version of temperature trend modeling, including year, region + season. Creates Figures 2 and 3 of manuscript, and Tables A2-A5 (model results). Also creates part of metadata table A1.
...
temp = mutate(temp, 
              WM = case_when(Month %in% c(10, 11, 12) ~ Month-9,
                             TRUE ~ Month +3),
              fMonthYear = as.factor(paste(WY, WM)),
              fMY2 = as.numeric(fMonthYear),
              MonthYear = WY+(WM-12)/12)

m.maxsAR1 =  glmmTMB(maxAvTemp ~ fWY + Season * Region2 + (1|Station)+ ar1(fMonthYear + 0|Station), data = temp)
summary(m.maxsAR1)
...
m.maxsAR1b =  glmmTMB(maxAvTemp ~ fWY + Season * Region2 + (1|Station)+ ar1(fMonthYear - 1|Station), data = temp)
summary(m.maxsAR1b)

library(nlme)

#do it with a different package
m.maxsAR2 = lme(maxAvTemp~fWY + Season * Region2, random=~1|Station, data=temp, correlation=corAR1(.5, ~fMY2|Station))
summary(m.maxsAR2)

#did we fix it?
acf(residuals(m.maxs9))
acf(residuals(m.maxsAR1))
acf(residuals(m.maxsAR1b))
acf(residuals(m.maxsAR2))
#that one isn't much better. Not sure why.
...
(plotmax9.rs = plot(max9.emm2, comparisons = TRUE, plotit = T, adjust = "bonferroni", alpha = 0.001))
# add letters
...
tiff(filename = "manuscript_code/Figures/Fig2_Watertemp_trends_max.tiff", height = 8.5, width = 7,units = "in", 
    res = 300, pointsize = 12, family = "sans")
(updated/maxplot + plot_layout(heights = c(2,6.5)))
dev.off()
```
...
# Used this one for mansucript
tiff(filename = "manuscript_code/Figures/Fig3_Watertemp_trends_min.tiff", height = 7.5, width = 7,units = "in", 
    res = 300, pointsize = 12, family = "sans")
p4 + (plot_ci_min9_wy/plot_ci_min9) + plot_layout(widths = c(1,25))
