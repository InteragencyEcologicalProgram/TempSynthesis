...
```{r}
latlon <- read_csv("manuscript_code/Data/StationsMetadata.csv")%>%
  dplyr::select(c(Station, Latitude, Longitude))
heatDays250 <- read_csv("manuscript_code/Data/heatDaysHighMedLow25_20210811.csv")
heatDays210 <- read_csv("manuscript_code/Data/heatDaysHighLow21_20210904.csv")
noRecov250 <- read_csv("manuscript_code/Data/noRecovery25_20210811.csv")
noRecov210 <- read_csv("manuscript_code/Data/noRecovery21_20211118.csv")
tempDaily0 <- readRDS("manuscript_code/Data/tempDaily.rds") %>%
...
Look at high stress no recovery data
* a couple of high points in the SE causing weird modeling results 
```{r}
ggplot(noRecov25) + geom_point(aes(x = Longitude, y = Latitude, color = propDays), size = 2.5) + facet_wrap(~Anomaly) + scale_color_viridis() + theme_bw()
```
...
```{r}
regions = read_sf(here::here("manuscript_code/RosiesRegionsEdited/Rosies_regions_edited.shp"))
regions = st_transform(regions, crs = 4326)

delta = read_sf("manuscript_code/DeltaShapefile/hydro_delta_marsh.shp")
...
stas = read.csv("manuscript_code/Data/StationsMetadata.csv") %>%
...
# save(deltabuff, file="manuscript_code/Data/gam/deltabuffthick.Rdata")

```

Change station into sf
```{r}
load("manuscript_code/Data/gam/deltabuffthick.Rdata")
...
#### Save/load files
```{r}
save(delta2, stas, newdatag21abin, newdatag25abin, newdatag21NRabin,  newdatag25NRabin, file = "manuscript_code/Data/gam/newdataPreds_annualanomaly_20230613.RData", compress = "xz")

load("manuscript_code/Data/gam/newdataPreds_annualanomaly_20230613.RData")
```
...
#### Save raster files
```{r}
save(delta2, rastered_preds21abin, rastered_preds25abin, rastered_preds21NRabin, rastered_preds25NRabin, file = "manuscript_code/Data/gam/RasteredPreds_20230613.RData", compress = "xz")
```

#### Start here if just want to plot
```{r}
load("manuscript_code/Data/gam/RasteredPreds_20230613.RData")
load("manuscript_code/Data/gam/deltabuffthick.Rdata")
```
...
Crop 
* This is slow, about 20-30 mins per crop
```{r}
...
Save crops
```{r}
# save(raster25Diff_crop, raster21Diff_crop, raster21NRDiff_crop, file = "manuscript_code/Data/gam/RasterDiff_part1_20230503.RData", compress = "xz")
# load("manuscript_code/Data/gam/RasterDiff_part1_20230503.RData")

save(raster25Diff_crop, raster21Diff_crop, raster21NRDiff_crop, raster25NRDiff_crop, file = "manuscript_code/Data/gam/RasterDiff_20230503.RData", compress = "xz")
```

```{r}
load("manuscript_code/Data/gam/RasterDiff_20230503.RData")
```
...
```{r}
raster_plotDayDiff<-function(data, labels="All", limit1, limit2, by){

 # lims = c(min(data$Prediction, na.rm = T), max(data$Prediction, na.rm = T))

 ggplot()+
  geom_stars(data=data)+
   scale_x_continuous(breaks = seq(from = 550000, to = 670000, by = 40000), expand = c(0,0))+
  scale_fill_viridis_c(option = "plasma", breaks = seq(from = limit1, to = limit2, by = by), na.value="white",limits = c(limit1, limit2))+
   # facet_wrap(~Date)+
    coord_sf()+
    ylab("Latitude")+
    xlab("Longitude")+
    ...
    guides(fill=guide_colorbar(title="Days Difference"))
...
```{r}
range(raster21Diff_crop$Prediction2, na.rm=TRUE)
range(raster25Diff_crop$Prediction2, na.rm=TRUE)

range(raster21NRDiff_crop$Prediction2, na.rm=TRUE)
range(raster25NRDiff_crop$Prediction2, na.rm=TRUE)

(raster21DiffPlot <- raster_plotDayDiff(data = raster21Diff_crop, limit1 = -10, limit2 = 45, by = 15))
(raster25DiffPlot <- raster_plotDayDiff(data = raster25Diff_crop, limit1 = -15, limit2 = 45, by = 15))
(raster21NRDiffPlot <- raster_plotDayDiff(raster21NRDiff_crop, limit1 = 0, limit2 = 45, by = 15))
(raster25NRDiffPlot <- raster_plotDayDiff(raster25NRDiff_crop,limit1 = -10, limit2 = 85, by = 15))
```
...
* This is very slow, took ~ 5 hours?
```{r}
...
#### Save and load here
```{r}
save(plot21_anom, plot21_avg, plot25_anom, plot25_avg, plot21NR_anom, plot21NR_avg, plot25NR_anom, plot25NR_avg, raster21DiffPlot, raster25DiffPlot, raster21NRDiffPlot, raster25NRDiffPlot, file= "manuscript_code/Data/gam/finalplots_20230613.RData")
```

```{r}
load("manuscript_code/Data/gam/finalplots_20230613.RData")
```
...
##### Theme
```{r}
theme_fig <-ggplot2::theme(plot.title=element_text(size=12), 
...
(plot21_avg <- plot21_avg  + 
    scale_fill_viridis_c(option = "viridis", breaks = seq(from = 0, to = 120, by = 20), na.value="white", limits = c(0,110)) + 
    annotate(geom = "text", x = 580000, y = 4.255e+06, label = "A", size = 6) + 
...
(plot21_anom2 <- plot21_anom + 
       annotate(geom = "text", x = 580000, y = 4.255e+06, label = "B", size = 6) + 
    theme_fig +
    theme(legend.position = "none") +
    scale_fill_viridis_c(option = "viridis", breaks = seq(from = 0, to = 120, by = 20), na.value="white", limits = c(0,110)) +
...
(plot25_avg2 <- plot25_avg + 
    scale_fill_viridis_c(option = "viridis", breaks = seq(from = 0, to = 120, by = 20), na.value="white", limits = c(0,110)) +
       annotate(geom = "text", x = 580000, y = 4.255e+06, label = "D", size = 6) + 
    theme_fig + 
    theme(legend.position = "none"))
...
(plot25_anom2 <- plot25_anom + 
    scale_fill_viridis_c(option = "viridis", breaks = seq(from = 0, to = 120, by = 20), na.value="white", limits = c(0,110)) + 
       annotate(geom = "text", x = 580000, y = 4.255e+06, label = "E", size = 6) + 
    theme_fig +
    theme(legend.position = "none"))
...
(raster21DiffPlot2 <- raster21DiffPlot + 
    ...
    scale_y_continuous(expand = c(0,0)) +
       annotate(geom = "text", x = 580000, y = 4.255e+06, label = "C", size = 6) + 
    theme_fig +
    theme(legend.position = "none") +
...
(raster25DiffPlot2 <- raster25DiffPlot + 
    scale_x_continuous(expand = c(0,0)) + 
    scale_y_continuous(expand = c(0,0)) +
       annotate(geom = "text", x = 580000, y = 4.255e+06, label = "F", size = 6) + 
    theme_fig +
    theme(legend.position = "none"))
...
plot21_avg3 <- annotate_figure(plot21_avg2, left = text_grob("21°C", color = "black", rot = 90, size = 14))

plot25_avg3 <- annotate_figure(plot25_avg2,
  left = text_grob("25°C", color = "black", rot = 90, size = 14))

(stress <- grid.arrange(legend1, plot21_avg3, plot21_anom2, plot25_avg3, plot25_anom2, 
                        nrow = 3, 
                        layout_matrix = rbind(c(1,1), c(2,3), c(4,5)),
                        widths = c(5,5,5), heights = c(1,3.3,3)))
...
##### Final Stress plot
```{r}
tiff(filename=file.path("manuscript_code/Figures/StressPlot.tiff"), units="in",type="cairo", bg="white", height=6.5, 
     width=8, res=300, pointsize=10, compress = "lzw")
(stressPlotC <- grid.arrange(legend1, legend2, plot21_avg3, plot21_anom2, raster21DiffPlot2, plot25_avg3, plot25_anom2, raster25DiffPlot2,
            ...
            layout_matrix = rbind(c(1,1,2), c(3,4,5), c(6,7,8)),
             widths = c(3.3,3,3), heights = c(0.75,3.3,3.05)))
dev.off()
```
...
(plot21NR_avg2 <- plot21NR_avg  + 
    ...
    theme_fig +
    theme(legend.position = "none") + 
    labs(title = "A) Average Year"))

(plot21NR_anom2 <- plot21NR_anom + 
    theme_fig +
    theme(legend.position = "none") +
    scale_fill_viridis_c(option = "viridis", breaks = seq(from = 0, to = 120, by = 20), na.value="white", limits = c(0,110)) +
    labs(title = "B) Warm Year (+1°C)"))
...
(plot25NR_anom2 <- plot25NR_anom + 
    scale_fill_viridis_c(option = "viridis", breaks = seq(from = 0, to = 120, by = 25), na.value="white", limits = c(0,110)) +
    theme_fig +
    theme(legend.position = "none"))
...
```{r}
tiff(filename=file.path("manuscript_code/Figures/StressNRPlot_updated.tiff"), units="in",type="cairo", bg="white", height=3.5, 
     width=8, res=300, pointsize=10, compress = "lzw")
...
#                             nrow = 3,
#             layout_matrix = rbind(c(1,1,2), c(3,4,5), c(6,7,8)),
#              widths = c(3.23,3,2.85), heights = c(0.75,3,3))

stressPlotD <- grid.arrange(legend3, legend4, plot21NR_avg3, plot21NR_anom2, raster21NRDiffPlot2, 
                            nrow = 2,
            layout_matrix = rbind(c(1,1,2), c(3,4,5)), 
             widths = c(3.3,3,3), heights = c(0.75,3))
dev.off()
```
...
viridis::mako(6)
Info25 <- noRecov25 %>%
  mutate(AnomalyGrp = ifelse(Anomaly>0.4, "High", "Low"),
         Region = replace(Region, Region == "San Joaquin", "Central"),
         Region = replace(Region, Region == "Sac River", "Confluence")) %>%
  mutate(Region = factor(Region, levels = c("North Delta", "Confluence", "Central", "South", "Suisun Bay", "Suisun Marsh")))

Range <- Info25 %>%
  group_by(AnomalyGrp, Region) %>%
  summarize(min = min(nDaysAdj),
            mean = mean(nDaysAdj),
            max = max(nDaysAdj),
            sd = sd(nDaysAdj))
(Anomaly25NRPlot <- ggplot(Info25) + 
  geom_boxplot(aes(x = Region, y = nDays, fill = AnomalyGrp)) + 
  labs(y = "Number of Days of Heat Stress at 25°C", fill = "Anomaly") + 
  scale_fill_manual(values = c("#3497A9FF", "#DEF5E5FF")) +
...
```{r}
png(filename=file.path("manuscript_code/Figures/Anomaly25NRPlot_updated.png"), units="in",type="cairo", bg="white", height=4, 
...
## Get estimates of ndays by region
```{r}
# predicted data

# Model for 21 days
newdatag21abin$Prediction2 = newdatag21abin$Prediction*365
df_data21 <- as.data.frame(newdatag21abin)
df_data21_sf <- df_sf <- st_as_sf(df_data21, coords = c("Longitude", "Latitude"), crs = 4326)
regions <- st_read(here::here("manuscript_code", "RosiesRegionsEdited", "Rosies_regions_edited.shp"))
deltabuff_4326 <- st_transform(deltabuff, st_crs(df_data21_sf))

df_data21_crop <- st_crop(df_data21_sf, deltabuff_4326)
head(df_data21_crop)
df_data21_sf_regions <- st_join(df_data21_crop, regions, join = st_within)

df_data21_diff <- df_data21_sf_regions %>%
  as.data.frame() %>%
  filter(!is.na(Regions)) %>%
  dplyr::select(-Prediction, -SE, -L95, -U95) %>%
  pivot_wider(names_from = "Anomaly",values_from = "Prediction2", id_cols = c("Location", "Latitude_s", "Longitude_s", "Regions")) %>%
  mutate(diff = `1`-`0`) %>%
  pivot_longer(cols = c(`0`:diff), names_to = "Anom", values_to = "value") %>%
  group_by(Anom) %>%
  mutate(meanAnom= mean(value, na.rm = TRUE),
         sdAnom = sd(value, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(Regions, Anom, meanAnom, sdAnom) %>%
...
write_csv(df_data21_diff, "manuscript_code/Data/gam/stressgam_predictions_summarized_21.csv")



# Model for 25 days
newdatag25abin$Prediction2 = newdatag25abin$Prediction*365
df_data25 <- as.data.frame(newdatag25abin)
df_data25_sf <- df_sf <- st_as_sf(df_data25, coords = c("Longitude", "Latitude"), crs = 4326)
deltabuff_4326 <- st_transform(deltabuff, st_crs(df_data25_sf))

df_data25_crop <- st_crop(df_data25_sf, deltabuff_4326)
head(df_data25_crop)
df_data25_sf_regions <- st_join(df_data25_crop, regions, join = st_within)

df_data25_diff <- df_data25_sf_regions %>%
  as.data.frame() %>%
  filter(!is.na(Regions)) %>%
  dplyr::select(-Prediction, -SE, -L95, -U95) %>%
  pivot_wider(names_from = "Anomaly",values_from = "Prediction2", id_cols = c("Location", "Latitude_s", "Longitude_s", "Regions")) %>%
...
write_csv(df_data25_diff, "manuscript_code/Data/gam/stressgam_predictions_summarized_25.csv")
```
