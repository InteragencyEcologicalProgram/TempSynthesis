...
fieldtemps <- read_csv("SpeciesTables/surveymaxtemps.csv") %>%
  select(Taxon = Taxa,
         LifeStage,
         q3, maxTemp)
...
daysExceedanceSummary <- DaysExceedanceDataUpper %>%
  ...
  distinct() %>%
  left_join(thresholds %>% select(Species, LifeStage, Taxon, Suboptimum_Upper, Tolerance_Upper))
...
write_csv(summary_all, here::here("manuscript_code","Data", "tables", "Table_AppendixB.csv"))
