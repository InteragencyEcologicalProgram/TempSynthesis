...
Model with year
```{r}
gamRRI2 <- bam(RelTempDiff~ te(julian, Hour, fWY,  d = c(2,1), k = c(15,7), bs = c("cc", "tp")), data = RelTempDiff_RRI, family = scat, method = "fREML", nthreads = 3, discrete = TRUE)
...
pcool <- AnomalyPlot(RRIPlot1, Anomaly = -1.5)
pavg <- AnomalyPlot(RRIPlot1, Anomaly = 0)
pwarm <- AnomalyPlot(RRIPlot1, Anomaly = 1.5)
...
ggplotly(ggplot(meanMonthly, aes(color = fWY)) +
