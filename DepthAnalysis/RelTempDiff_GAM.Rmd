...
reltempdiffplot <- ggplot(relativetempdiff, aes(x= julian, y = RelTempDiff))+ 
  geom_smooth(aes(color = TOD), method = "gam", formula = y~s(x, k = 5, bs = "cc"), se = T, fullrange = T) +
  facet_wrap(~Station) + 
  theme_bw() + theme(
    strip.text = element_text(size = 13, colour = "white", face = "bold"),
    strip.background = element_rect(fill = "black"),
...
Write plot
```{r}
tiff(filename=file.path("../Figures/Depth/Figure_AllStation_Gamsmooths.tiff"), units="in",type="cairo", bg="white", height=6, 
...
# Plot relative temp diff by year
```{r}
...
Original anomaly calculation (don't use this)
```{r}
RelTempDiff_RRI <- RelTempDiff_RRI %>%
  group_by(julian, Hour) %>%
  mutate(meanTemp = mean(Surface),
...
#### Model by Year and hour (no anomaly)
```{r}
...
### Exploring surface and bottom temps by year
```{r}
...
```{r}
save(predictions1, predictions2, MRZPlot, RRIPlot1, file = "DepthAnalysis/RelTempDiffGAM_20211008.RData")

write.csv(predictions1, "DepthAnalysis/RRI_Predictions.csv")
write.csv(RelTempDiff_RRI, "DepthAnalysis/RRI_reldiff.csv")
```
...
MRZfacet <- ggplot(subset(MRZPlot, Anomaly %in% c(-1.5, 0, 1.5)), aes(x = julian, y = Hour, z = fit)) +
  ...
  scale_y_continuous(expand = c(0,0))+
    #labs(title = Anomaly)+
    guides(fill=guide_colorbar(title="Bottom-Surface Temperature Difference", barwidth = 12))
...
```{r}
#Print out figure
tiff(filename=file.path("Figures/Depth/Figure_DepthGAM.tiff"), units="in",type="cairo", bg="white", height=5, 
     width=7, res=300, pointsize=10,compression="lzw")
grid.arrange(legendF, RRIfacet2, MRZfacet2, nrow = 3, 
            layout_matrix = rbind(1, 2, 3),
             widths = c(2), heights = c(0.5,2,2))
dev.off()
```
