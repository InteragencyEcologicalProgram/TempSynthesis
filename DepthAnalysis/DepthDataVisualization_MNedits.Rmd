...
```{r timezone}
library(readr)
depth2 <- read_rds("DepthAnalysis/Data/depthdata_heatstress_20200728.rds") %>%
  filter(WaterCol == "Bottom",
         Station == "ANH")

ANH1 <- read_csv("DepthAnalysis/Data/ANH_07122020_12312019_BottomTemp.csv")
ANH2 <- read_csv("DepthAnalysis/Data/ANH_07122020_12312019_BottomTemp.csv",
                 col_types = cols( DATE = col_character())) %>%
  mutate(Temp2 = round(VALUE,1),
         Datetime = parse_datetime(DATE, format = "%m/%d/%Y %H:%M"),
...
CheckANH <- left_join(depth2, ANH2)

library(plotly)
ANHp <- ggplot(ANH2, aes(x = Datetime, y = Temp2, color = factor(Hour))) + geom_point()
ggplotly(ANHp)
```
...
Max.time.summary = Max.time %>%
  ...
  summarize(Mean.max.hour = mean(Hour))

ggplot(Max.time.summary, aes(x= ordered(Month), y = Mean.max.hour)) + facet_grid(Station~WaterCol) + geom_col() + 
  theme_bw() + 
...
Max.time.summary2 = Max.time %>%
  group_by(Station, WaterCol) %>%
  summarize(Mean.max.hour = round(mean(Hour),1))

ggplot(Max.time.summary2, aes(x= Station, y = Mean.max.hour)) + facet_grid(~WaterCol) + geom_col() + 
  theme_bw() + 
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 15),
        strip.text = element_text(size = 15))

ggplot(Max.time.summary, aes(x= Station, y = Mean.max.hour)) + facet_grid(~WaterCol) + geom_boxplot() + 
  theme_bw() + 
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 15),
        strip.text = element_text(size = 15))
...
#plot by month
ggplot(depthDiff_rel3, aes(x = factor(Month), y = RelTempDiff,colour=factor(TOD))) + geom_point(size=.4, position=position_dodge(width=0.2)) + labs(y="Relative TempDiff (c)") +geom_smooth(aes(group = factor(TOD), colour=factor(TOD), method = "gam", se = T))+
...
ggplot(depthDiff_rel3, aes(x = factor(Month), y = RelTempDiff,colour=factor(TOD)))+ labs(y="Relative TempDiff (c)") +geom_smooth(aes(group = factor(TOD), colour=factor(TOD), method = "loess", se = T))+
...
ggplot() + geom_point(data = MRZ_depth,aes(x = Date, y = Temp, color = WaterCol), alpha = 0.5, size = 0.5) + facet_wrap(~factor(TOD)) + labs(title = "MRZ Absolute Temps", y="Water Temp (C)") +
...
ggplot() + geom_point(data = RRI_depth,aes(x = Date, y = Temp, color = WaterCol), alpha = 0.5, size = 0.5) + facet_wrap(~factor(TOD)) + labs(title = "RRI Absolute Temps", y="Water Temp (C)") +
  geom_hline(yintercept = 25, color = "black", linetype='dashed') +
    scale_color_brewer(palette = "Dark2") +
    theme_bw() +
      theme(axis.text = element_text(size = 15),
