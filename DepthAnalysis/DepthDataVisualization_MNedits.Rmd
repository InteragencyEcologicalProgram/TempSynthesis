...
```{r import}
depth <- readRDS("Data/depthdata_heatstress_20201005.rds")
#depth$Datetime <- attr(depth$Datetime, "tzone") <- "PDT"
```

### Check timezone
```{r timezone}
depth2 <- read_rds("Data/depthdata_heatstress_20201005.rds") %>%
...
#plot by month
ggplot(depthDiff_rel3, aes(x = factor(Month), y = RelTempDiff,colour=factor(TOD))) + geom_point(size=.4, position=position_dodge(width=0.2)) + labs(y="Relative TempDiff (c)") +geom_smooth(aes(group = factor(TOD), colour=factor(TOD), method = "gam", se = TRUE))+
