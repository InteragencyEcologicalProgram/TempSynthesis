...
## mins, max, medians for manuscript
mean(relativetempdiff$Surface, na.rm = TRUE)
min(relativetempdiff$Surface, na.rm = TRUE)
summary(relativetempdiff$Surface)
...
#first with Hour
plotANHhour <- ggplot(RelTempDiff_ANH, aes(x= julian, y = RelTempDiff, color = factor(Hour)))+ geom_point(size = .6) 
...
## there are really 3 variables x = julian day y = reltempdiff and the factor is TOD... is that why I am having so much trouble combing?
```
