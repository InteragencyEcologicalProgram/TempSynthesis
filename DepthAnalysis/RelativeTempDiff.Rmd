...
# Model
gamRRI_0 <- bam(RelTempDiff~ te(julian, Hour, tempAnomaly, k = c(12,6,16), bs = c("cc", "cc", "tp")), 
...
gamRRI <- bam(RelTempDiff~ te(julian, Hour, tempAnomaly, k = c(12,6,16), bs = c("cc", "cc", "tp")), data = RelTempDiff_RRI, 
            family = scat, 
            method = "fREML", 
            rho = rAn, 
...
summary(gamRRI)
vis.gam(x = gamRRI,
        view = c("julian", "tempAnomaly"),
        color = "heat", plot.type = "contour")

save(gamRRI, file = "gamRRI.RData")
...
gAnRRIa <- bam(Surface~ s(julian, k = 20, bs = "cc") + s(Hour, k = 10, bs = "cc"), 
               data = RelTempDiff_RRI,
               method = "fREML", 
               nthreads = 3, 
               discrete = TRUE)

hist(RelTempDiff_RRI$Surface)
hist(RelTempDiff_MAL$Surface)

ggplot(RelTempDiff_RRI, aes(x = factor(Month), y = Surface)) + geom_boxplot()
...
gAnRRI <- bam(Surface~ s(julian, k = 20, bs = "cc") + s(Hour, k = 10, bs = "cc"),
              data = RelTempDiff_RRI, 
              method = "fREML", 
              rho = rAn, 
              AR.start = RelTempDiff_RRI$start.event, 
